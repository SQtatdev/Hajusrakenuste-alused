...
response = requests.get('https://www.google.com')  # Метод GET
...
response = requests.get('https://www.google.com', headers=headers)  # Запрос с заголовками

# 3. Отправляем данные (setMethod: POST, send)
data = {'key': 'value'}
response = requests.post('https://www.google.com', data=data)  # Метод POST с данными
